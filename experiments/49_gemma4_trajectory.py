"""Run Gemma/Pythia UDC/TLE evaluation on BENCH-2 and TruthfulQA.

This is the hackathon-oriented new-architecture runner. It is intentionally
lighter than the full paper experiments but preserves the controlled-evaluation
logic:

- BENCH-2 matched pairs on HaluEval QA
- TruthfulQA scope check
- UDC and TLE side-by-side
- length-only baseline
- per-example CSV + summary JSON
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate UDC/TLE on a new architecture.")
                ...
                rows.append(
                    {
                        ...
                        "udc_mean_tok_mean": result.udc_scalar,
                        "tle_scalar": result.tle_scalar,
                        "tle_mean_tok_mean": result.tle_scalar,
                        "input_format": result.input_format,
                        "verdict_default": result.verdict,
                    }
                )
            except Exception as err:  # keep the run alive on individual failures
                errors.append(
                    {
                        "pair_id": pair_idx,
...
def summarize_rows(df: pd.DataFrame, dataset_name: str) -> dict:
    ...
    summaries = {}
    for metric_col in ["udc_scalar", "tle_scalar", "response_length"]:
        ...
        summaries[metric_col] = summary.to_dict()
...
def main() -> None:
    ...
    print("=" * 72)
    print("EXP 49 — NEW ARCHITECTURE UDC/TLE EVAL")
    ...
    bench_rows_path = outdir / "49_gemma4_bench2_rows.csv"
    bench_json_path = outdir / "49_gemma4_bench2.json"
    bench_df.to_csv(bench_rows_path, index=False)

    bench_summary = summarize_rows(bench_df, "bench2")
    bench_summary.update(
        {
            "experiment": "49_gemma4_trajectory",
            "model": args.model,
    ...
    print(json.dumps(bench_summary, indent=2)[:3000])
    ...
    truthful_rows_path = outdir / "49_gemma4_truthfulqa_rows.csv"
    truthful_json_path = outdir / "49_gemma4_truthfulqa.json"
    ...
    print(json.dumps(truthful_summary, indent=2)[:3000])
